...
import { supabase } from "../../supabaseClient"; // Corrected path to supabase

// FIX: Go up TWO levels to reach src/assets
...
export default function Header({
  ...
  isDarkMode,
  toggleTheme,
  userProfile, // Pass this from SKHomeView to get the real name
}) {
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: "Weekly backup successful.",
      time: "2m ago",
      type: "success",
      read: false,
    },
          ...
          <div className="flex items-center gap-3 border-r border-[#E7F0FA] dark:border-gray-700 pr-6 mr-2">
            ...
            <div className="hidden md:block">
              ...
              <p className="text-[10px] uppercase font-bold tracking-wider text-[#7BA4D0] dark:text-gray-400">
                {userProfile?.requested_barangay || "Barangay"} Portal
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4 z-20">
          ...
          <div className="relative">
            <button
              ...
              className={`relative p-2 rounded-full transition-all ${isNotifOpen ? "bg-[#E7F0FA] dark:bg-gray-700 text-[#2E5E99] dark:text-white" : "text-[#7BA4D0] hover:text-[#2E5E99] dark:text-gray-400 dark:hover:text-white"}`}
            >
              <Bell className="w-5 h-5" />
              {unreadCount > 0 && (
                <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full border border-white dark:border-[#1e293b]"></span>
              )}
            </button>

            {isNotifOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-[#1e293b] rounded-xl shadow-xl border border-[#E7F0FA] dark:border-gray-700 py-2 overflow-hidden">
                ...
                <div className="max-h-64 overflow-y-auto">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className="px-4 py-3 border-b border-[#E7F0FA] dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                    >
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="w-10 h-10 bg-gradient-to-br from-[#2E5E99] to-[#0D2440] dark:from-blue-600 dark:to-blue-900 rounded-full flex items-center justify-center text-white font-bold shadow-md">
                {userProfile?.full_name?.substring(0, 2).toUpperCase() || "SK"}
              </div>
              <ChevronDown
            ...
            {isProfileOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#1e293b] rounded-xl shadow-xl border border-[#E7F0FA] dark:border-gray-700 py-2">
